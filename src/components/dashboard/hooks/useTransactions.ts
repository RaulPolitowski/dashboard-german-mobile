...
const mapFinancialToTransactions = (financialData: any[]): Transaction[] => {
  const inflowTransactions: Transaction[] = financialData.map(item => ({
    id: item.id.toString(),
  ...
  const outflowTransactions: Transaction[] = financialData.map(item => ({
    id: `out-${item.id}`,
    date: item.date || new Date().toISOString(),
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Combinar todos os dados usando spread operator em vez de concat
